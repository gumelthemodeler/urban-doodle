...
function TradeMenu.Init(parentFrame, tooltipMgr)
	...
	Network.TradeUpdate.OnClientEvent:Connect(function(action, data)
		...
		if action == "Open" then
			...
			local mg = Instance.new("UIGridLayout", myOfferFrame); mg.CellSize = UDim2.new(0, 70, 0, 70); mg.CellPadding = UDim2.new(0,5,0,5)
			...
			local tg = Instance.new("UIGridLayout", theirOfferFrame); tg.CellSize = UDim2.new(0, 70, 0, 70); tg.CellPadding = UDim2.new(0,5,0,5)
			local tPad = Instance.new("UIPadding", theirOfferFrame); tPad.PaddingTop = UDim.new(0,5); tPad.PaddingLeft = UDim.new(0,5)

			-- [[ FIX: Graphic Inventory Selector replaces the text box on PC ]]
			...
			inventorySelector.BackgroundColor3 = Color3.fromRGB(12,12,15); inventorySelector.ScrollBarThickness = 4
			...
			local ig = Instance.new("UIGridLayout", inventorySelector); ig.CellSize = UDim2.new(0, 70, 0, 70); ig.CellPadding = UDim2.new(0,5,0,5)
			...
			confirmBtn.Position = UDim2.new(0.25, 0, 1, -55)
			confirmBtn.AnchorPoint = Vector2.new(0.5, 0)
			confirmBtn.Font = Enum.Font.GothamBlack
			confirmBtn.TextColor3 = Color3.fromRGB(150, 255, 150)
			...
			cancelBtn.Size = UDim2.new(0, 200, 0, 45)
			cancelBtn.Position = UDim2.new(0.75, 0, 1, -55)
			...
			cancelBtn.Text = "ABORT TRANSACTION"
		...
		elseif action == "Sync" then
			...
			if contentArea then
				...
				if myFr then 
					...
					myFr.CanvasSize = UDim2.new(0,0,0, math.ceil(cCount/3)*75 + 10)
				end
				if theirFr then 
					DrawOfferItems(theirFr, theirOffer.Items, false) 
					local cCount = 0; for k,v in pairs(theirOffer.Items) do cCount += 1 end
					theirFr.CanvasSize = UDim2.new(0,0,0, math.ceil(cCount/3)*75 + 10)
				end
				if invFr then 
					...
					invFr.CanvasSize = UDim2.new(0,0,0, math.ceil(cCount/7)*75 + 10)
				end
			end
