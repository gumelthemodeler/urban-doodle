...
-- [[ NEW: Cinematic Fade Transition ]]
local function PlayTransition(text, callback)
	local topGui = player.PlayerGui:FindFirstChild("AOT_Interface")
	local overlay = topGui and topGui:FindFirstChild("BattleTransition")
	if not overlay then callback(); return end

	local textLbl = overlay:FindFirstChild("TextLabel")
	overlay.Visible = true
	textLbl.Text = text

	local tIn = TweenService:Create(overlay, TweenInfo.new(0.4), {BackgroundTransparency = 0})
	local ttIn = TweenService:Create(textLbl, TweenInfo.new(0.4), {TextTransparency = 0})
	tIn:Play(); ttIn:Play()

	tIn.Completed:Connect(function()
		callback()
		task.wait(0.6) 
		local tOut = TweenService:Create(overlay, TweenInfo.new(0.5), {BackgroundTransparency = 1})
		local ttOut = TweenService:Create(textLbl, TweenInfo.new(0.5), {TextTransparency = 1})
		tOut:Play(); ttOut:Play()
		tOut.Completed:Connect(function() overlay.Visible = false end)
	end)
end
...
local function RenderStatuses(container, statuses)
	...
	local function addIcon(iconTxt, bgColor, strokeColor, tooltipText)
		local f = Instance.new("Frame", container); f.Size = UDim2.new(0, 24, 0, 18); f.BackgroundColor3 = bgColor; Instance.new("UICorner", f).CornerRadius = UDim.new(0, 4); Instance.new("UIStroke", f).Color = strokeColor
		...
		local hoverBtn = Instance.new("TextButton", f); hoverBtn.Size = UDim2.new(1, 0, 1, 0); hoverBtn.BackgroundTransparency = 1; hoverBtn.Text = ""; hoverBtn.ZIndex = 500
...
local function StartVisualTimer(endTime)
	...
	local remaining = endTime - os.time(); if remaining < 0 then remaining = 0 end
...
local function LoadLeaderboard()
	...
	task.spawn(function()
		...
		for _, entry in ipairs(lbData) do
			local fr = Instance.new("Frame", LeaderboardScroll); fr.Size = UDim2.new(1, -10, 0, 40); fr.BackgroundColor3 = Color3.fromRGB(30, 30, 35); Instance.new("UICorner", fr).CornerRadius = UDim.new(0, 4)
...
function PvPTab.Init(parentFrame, tooltipMgr)
	...
	LobbyFrame = Instance.new("Frame", parentFrame); LobbyFrame.Name = "PvPLobby"; LobbyFrame.Size = UDim2.new(1, 0, 1, 0); LobbyFrame.BackgroundTransparency = 1; LobbyFrame.Visible = false
	local QueuePanel = Instance.new("Frame", LobbyFrame); QueuePanel.Size = UDim2.new(1, 0, 0, 100); QueuePanel.BackgroundColor3 = Color3.fromRGB(15, 15, 20); Instance.new("UICorner", QueuePanel).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", QueuePanel).Color = Color3.fromRGB(100, 80, 40)

	-- [[ FIX: Added Fade Transition PvP Back Button ]]
	local PvPBackBtn = Instance.new("TextButton", QueuePanel)
	PvPBackBtn.Size = UDim2.new(0, 80, 0, 25); PvPBackBtn.Position = UDim2.new(0, 10, 0, 5); PvPBackBtn.Font = Enum.Font.GothamBold; PvPBackBtn.TextSize = 11; PvPBackBtn.TextColor3 = Color3.fromRGB(255, 255, 255); PvPBackBtn.Text = "<- BACK"
	...
	PvPBackBtn.MouseButton1Click:Connect(function()
		...
		PlayTransition("RETURNING TO COMBAT MENU...", function()
	...
	local QueueTitle = Instance.new("TextLabel", QueuePanel); QueueTitle.Size = UDim2.new(1, 0, 0, 30); QueueTitle.Position = UDim2.new(0, 0, 0, 10); QueueTitle.BackgroundTransparency = 1; QueueTitle.Font = Enum.Font.GothamBlack; QueueTitle.Text = "UNDERGROUND ARENA"; QueueTitle.TextColor3 = Color3.fromRGB(255, 215, 100); QueueTitle.TextSize = 22
	QueueBtn = Instance.new("TextButton", QueuePanel); QueueBtn.Size = UDim2.new(0.4, 0, 0, 40); QueueBtn.Position = UDim2.new(0.3, 0, 0.5, 0); QueueBtn.Font = Enum.Font.GothamBlack; QueueBtn.Text = "ENTER MATCHMAKING"; QueueBtn.TextColor3 = Color3.new(1,1,1); QueueBtn.TextSize = 16; ApplyButtonGradient(QueueBtn, Color3.fromRGB(60, 120, 200), Color3.fromRGB(30, 60, 100), Color3.fromRGB(40, 80, 140))
	...
	local SplitFrame = Instance.new("Frame", LobbyFrame); SplitFrame.Size = UDim2.new(1, 0, 1, -120); SplitFrame.Position = UDim2.new(0, 0, 0, 120); SplitFrame.BackgroundTransparency = 1; local sLayout = Instance.new("UIListLayout", SplitFrame); sLayout.FillDirection = Enum.FillDirection.Horizontal; sLayout.Padding = UDim.new(0, 10)
	...
	VS_Splash = Instance.new("Frame", parentFrame.Parent); VS_Splash.Size = UDim2.new(1, 0, 1, 0); VS_Splash.BackgroundColor3 = Color3.fromRGB(0, 0, 0); VS_Splash.BackgroundTransparency = 1; VS_Splash.Visible = false; VS_Splash.ZIndex = 500
	...
	ArenaFrame = Instance.new("Frame", parentFrame.Parent); ArenaFrame.Name = "PvPArenaFrame"; ArenaFrame.Size = UDim2.new(0, 750, 0, 520); ArenaFrame.Position = UDim2.new(0.5, 0, 0.5, 0); ArenaFrame.AnchorPoint = Vector2.new(0.5, 0.5); ArenaFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 20); ArenaFrame.Visible = false; ArenaFrame.ZIndex = 200; Instance.new("UICorner", ArenaFrame).CornerRadius = UDim.new(0, 12); Instance.new("UIStroke", ArenaFrame).Thickness = 2; Instance.new("UIStroke", ArenaFrame).Color = Color3.fromRGB(255, 50, 50)
	...
	local FeedBox = Instance.new("Frame", ArenaFrame); FeedBox.Size = UDim2.new(0.96, 0, 0, 90); FeedBox.BackgroundColor3 = Color3.fromRGB(22, 22, 26); FeedBox.ClipsDescendants = true; FeedBox.LayoutOrder = 3; Instance.new("UICorner", FeedBox).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", FeedBox).Color = Color3.fromRGB(60, 60, 70)
	...
	ActionGrid = Instance.new("ScrollingFrame", BottomArea); ActionGrid.Size = UDim2.new(1, 0, 1, 0); ActionGrid.BackgroundTransparency = 1; ActionGrid.ScrollBarThickness = 0; ActionGrid.BorderSizePixel = 0; local gridLayout = Instance.new("UIGridLayout", ActionGrid); gridLayout.CellSize = UDim2.new(0, 170, 0, 45); gridLayout.CellPadding = UDim2.new(0, 8, 0, 12); gridLayout.SortOrder = Enum.SortOrder.LayoutOrder; gridLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	...
	local CancelBtn = Instance.new("TextButton", InfoPanel); CancelBtn.Size = UDim2.new(0.7, 0, 0, 40); CancelBtn.Position = UDim2.new(0, 20, 1, -55); CancelBtn.Font = Enum.Font.GothamBlack; CancelBtn.TextColor3 = Color3.fromRGB(255, 255, 255); CancelBtn.TextSize = 14; CancelBtn.Text = "CANCEL"; ApplyButtonGradient(CancelBtn, Color3.fromRGB(160, 60, 60), Color3.fromRGB(100, 30, 30), Color3.fromRGB(60, 20, 20)); CancelBtn.MouseButton1Click:Connect(function() TargetMenu.Visible = false; ActionGrid.Visible = true; pendingSkillName = nil end)
	...
	local function CreateLimb(name, size, pos, hoverText, baseColor)
		...
		local mTop = Color3.new(math.clamp(baseColor.R * 0.6, 0, 1), math.clamp(baseColor.G * 0.6, 0, 1), math.clamp(baseColor.B * 0.6, 0, 1)); local mBot = Color3.new(math.clamp(baseColor.R * 0.3, 0, 1), math.clamp(baseColor.G * 0.3, 0, 1), math.clamp(baseColor.B * 0.3, 0, 1)); ApplyButtonGradient(limb, mTop, mBot, baseColor)
		limb.MouseEnter:Connect(function() local hTop = Color3.new(math.clamp(baseColor.R * 1.2, 0, 1), math.clamp(baseColor.G * 1.2, 0, 1), math.clamp(baseColor.B * 1.2, 0, 1)); local hBot = Color3.new(math.clamp(baseColor.R * 0.8, 0, 1), math.clamp(baseColor.G * 0.8, 0, 1), math.clamp(baseColor.B * 0.8, 0, 1)); ApplyButtonGradient(limb, hTop, hBot, baseColor); tHoverTitle.Text = name:upper(); tHoverTitle.TextColor3 = baseColor; local grad = tHoverTitle:FindFirstChildOfClass("UIGradient"); if grad then grad.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, hTop), ColorSequenceKeypoint.new(1, hBot)} end; tHoverDesc.Text = hoverText end)
		limb.MouseLeave:Connect(function() ApplyButtonGradient(limb, mTop, mBot, baseColor); tHoverTitle.Text = "SELECT TARGET"; tHoverTitle.TextColor3 = Color3.fromRGB(255, 215, 100); local grad = tHoverTitle:FindFirstChildOfClass("UIGradient"); if grad then grad.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 215, 100)), ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 150, 50))} end; tHoverDesc.Text = "Hover over a limb to see its tactical advantage." end)
		limb.MouseButton1Click:Connect(function()
			if pendingSkillName and not inputLocked then
				if EffectsManager.PlaySFX then EffectsManager.PlaySFX("Click") end
				LockGridAndWait(); Network.PvPAction:FireServer("SubmitMove", currentMatchId, pendingSkillName, name)
			end
		end)
	end
	Instance.new("UIAspectRatioConstraint", BodyContainer).AspectRatio = 0.8
	CreateLimb("Eyes", UDim2.new(0.24, 0, 0.18, 0), UDim2.new(0.5, 0, 0.08, 0), "Deals 20% Damage. Inflicts Weakness.", Color3.fromRGB(120, 120, 180)); CreateLimb("Nape", UDim2.new(0.24, 0, 0.06, 0), UDim2.new(0.5, 0, 0.22, 0), "Deals 150% Damage. Low accuracy.", Color3.fromRGB(220, 80, 80)); CreateLimb("Body", UDim2.new(0.48, 0, 0.38, 0), UDim2.new(0.5, 0, 0.45, 0), "Deals 100% Damage. Standard accuracy.", Color3.fromRGB(80, 160, 80)); CreateLimb("Arms", UDim2.new(0.22, 0, 0.38, 0), UDim2.new(0.14, 0, 0.45, 0), "Deals 50% Damage. Inflicts Weakness.", Color3.fromRGB(180, 140, 60)); CreateLimb("Arms", UDim2.new(0.22, 0, 0.38, 0), UDim2.new(0.86, 0, 0.45, 0), "Deals 50% Damage. Inflicts Weakness.", Color3.fromRGB(180, 140, 60)); CreateLimb("Legs", UDim2.new(0.23, 0, 0.32, 0), UDim2.new(0.37, 0, 0.81, 0), "Deals 50% Damage. Inflicts Crippled.", Color3.fromRGB(80, 140, 180)); CreateLimb("Legs", UDim2.new(0.23, 0, 0.32, 0), UDim2.new(0.63, 0, 0.81, 0), "Deals 50% Damage. Inflicts Crippled.", Color3.fromRGB(80, 140, 180))
	...
	local function UpdatePvPActionGrid()
		...
		local function CreateBtn(sName, color, order)
			local sData = SkillData.Skills[sName]; if not sData then return end
			...
			if not btn then
				btn = Instance.new("TextButton", ActionGrid); btn.Name = "Btn_" .. sName; btn.RichText = true; btn.Font = Enum.Font.GothamBold; btn.TextSize = 12
				btn.MouseButton1Click:Connect(function()
					if not inputLocked then
						if EffectsManager.PlaySFX then EffectsManager.PlaySFX("Click") end
						if sName == "Surrender" then LockGridAndWait(); Network.PvPAction:FireServer("Surrender", currentMatchId)
						elseif sData.Effect == "Rest" or sData.Effect == "TitanRest" or sData.Effect == "Eject" or sData.Effect == "Transform" or sData.Effect == "Block" then LockGridAndWait(); Network.PvPAction:FireServer("SubmitMove", currentMatchId, sName, "Body")
						else pendingSkillName = sName; ActionGrid.Visible = false; TargetMenu.Visible = true end
					end
				end)
			end
			btn.Visible = true; btn.LayoutOrder = order or 10; ApplyButtonGradient(btn, color, Color3.new(color.R*0.7, color.G*0.7, color.B*0.7), color); btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.Text = sName:upper()
		end
		...
		if isTransformed then
			...
			for sName, sData in pairs(SkillData.Skills) do
				if sData.Requirement == pTitan or sData.Requirement == "AnyTitan" or sData.Requirement == "Transformed" then
					if sName ~= "Titan Recover" and sName ~= "Eject" and sName ~= "Titan Punch" and sName ~= "Titan Kick" and sName ~= "Transform" then CreateBtn(sName, Color3.fromRGB(60, 40, 60), sData.Order or orderIndex); orderIndex += 1 end
				end
			end
		else
			...
			for sName, sData in pairs(SkillData.Skills) do
				...
				if req == pStyle or req == pClan or (req == "Ackerman" and pClan == "Awakened Ackerman") or (req == "ODM" and isODM) then CreateBtn(sName, Color3.fromRGB(45, 40, 60), sData.Order or orderIndex); orderIndex += 1 end
			end
		end
	end
	...
	Network:WaitForChild("PvPUpdate").OnClientEvent:Connect(function(action, matchId, data1, data2, p1Id, p2Id, turnEndTime)
		if action == "MatchStarted" then
			if data1 == player.Name or data2 == player.Name then
				isQueued = false; QueueBtn.Text = "ENTER MATCHMAKING"; ApplyButtonGradient(QueueBtn, Color3.fromRGB(60, 120, 200), Color3.fromRGB(30, 60, 100), Color3.fromRGB(40, 80, 140))
				currentMatchId = matchId; isFighter = true; playerIsP1 = (data1 == player.Name)

				local topGui = parentFrame:FindFirstAncestorOfClass("ScreenGui")
				if topGui then if topGui:FindFirstChild("TopBar") then topGui.TopBar.Visible = false end; if topGui:FindFirstChild("NavBar") then topGui.NavBar.Visible = false end end
				...
				task.wait(1.5); TweenService:Create(VS_Splash, TweenInfo.new(0.5), {BackgroundTransparency = 1}):Play(); TweenService:Create(SplashText, TweenInfo.new(0.5), {TextTransparency = 1}):Play()
				...
				PlayerNameText.Text = playerIsP1 and data1 or data2; EnemyNameText.Text = playerIsP1 and data2 or data1
				PlayerHPBar.Size = UDim2.new(1, 0, 1, 0); EnemyHPBar.Size = UDim2.new(1, 0, 1, 0); PlayerGasBar.Size = UDim2.new(1, 0, 1, 0); EnemyGasBar.Size = UDim2.new(1, 0, 1, 0)
				LogText.Text = "<font color='#FFD700'><b>THE MATCH HAS BEGUN!</b></font>"; UpdatePvPActionGrid(); StartVisualTimer(turnEndTime)
			end
		elseif action == "TurnStrike" and currentMatchId == matchId then
			ShakeUI(data1.ShakeType); LogText.Text = data1.LogMsg
			if data1.SkillUsed then local attackerIsLeft = false; if playerIsP1 then attackerIsLeft = (data1.Attacker == PlayerNameText.Text) else attackerIsLeft = (data1.Attacker ~= PlayerNameText.Text) end; if EffectsManager.PlayCombatEffect then EffectsManager.PlayCombatEffect(data1.SkillUsed, attackerIsLeft, pAvatarBox, eAvatarBox, data1.DidHit) end end
			if data1.P1_HP then
				local myHP = playerIsP1 and data1.P1_HP or data1.P2_HP; local myMax = playerIsP1 and data1.P1_Max or data1.P2_Max; local enemyHP = playerIsP1 and data1.P2_HP or data1.P1_HP; local enemyMax = playerIsP1 and data1.P2_Max or data1.P1_Max; local myGas = playerIsP1 and data1.P1_Gas or data1.P2_Gas; local enemyGas = playerIsP1 and data1.P2_Gas or data1.P1_Gas
		...
		elseif action == "NextTurnStarted" and currentMatchId == matchId then
			if isFighter then UpdatePvPActionGrid(); StartVisualTimer(data2) end
		elseif action == "MatchEnded" and currentMatchId == matchId then
			if data1 == player.UserId then if EffectsManager.PlaySFX then EffectsManager.PlaySFX("Victory", 1) end
			elseif data1 ~= "Draw" then if EffectsManager.PlaySFX then EffectsManager.PlaySFX("Defeat", 1) end end
			...
			ActionGrid.Visible = false; LeaveBtn.Visible = true; isFighter = false
		end
	end)

	LobbyFrame:GetPropertyChangedSignal("Visible"):Connect(function()
		if LobbyFrame.Visible then LoadLeaderboard() end
	end)
end
