...
function InheritTab.Init(parentFrame, tooltipMgr)
	...
	MainFrame.Name = "InheritFrame"; MainFrame.Size = UDim2.new(1, 0, 1, 0); MainFrame.BackgroundTransparency = 1; MainFrame.Visible = false; MainFrame.ScrollBarThickness = 0
	...
	Title.Size = UDim2.new(1, 0, 0, 40); Title.BackgroundTransparency = 1; Title.Font = Enum.Font.GothamBlack; Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.TextSize = 26; Title.Text = "THE PATHS (INHERITANCE)"; Title.LayoutOrder = 1
	ApplyGradient(Title, Color3.fromRGB(255, 215, 100), Color3.fromRGB(255, 150, 50))

	-- [[ FIX: PC Split Container ]]
	...
	local function CreateGachaPanel(gType, order)
		...
		Panel.Size = UDim2.new(0.48, 0, 1, 0); Panel.BackgroundColor3 = Color3.fromRGB(15, 15, 18); Panel.LayoutOrder = order
		...
		PTitle.Size = UDim2.new(1, 0, 0, 40); PTitle.BackgroundTransparency = 1; PTitle.Font = Enum.Font.GothamBlack; PTitle.TextColor3 = Color3.fromRGB(255, 215, 100); PTitle.TextSize = 22; PTitle.Text = (gType == "Titan") and "TITAN INHERITANCE" or "CLAN LINEAGE"

		local ListContainer = Instance.new("ScrollingFrame", Panel)
		ListContainer.Size = UDim2.new(1, -20, 0, 290); ListContainer.Position = UDim2.new(0, 10, 0, 45); ListContainer.BackgroundTransparency = 1; ListContainer.ScrollBarThickness = 6; ListContainer.AutomaticCanvasSize = Enum.AutomaticSize.Y
		...
		if gType == "Titan" then
			...
			for _, drop in ipairs(sortedTitans) do
				...
				local titleLbl = Instance.new("TextLabel", card); titleLbl.Size = UDim2.new(1, -20, 0, 20); titleLbl.Position = UDim2.new(0, 10, 0, 10); titleLbl.BackgroundTransparency = 1; titleLbl.Font = Enum.Font.GothamBold; titleLbl.TextColor3 = Color3.fromRGB(230, 230, 230); titleLbl.TextSize = 15; titleLbl.TextXAlignment = Enum.TextXAlignment.Left; titleLbl.RichText = true; titleLbl.Text = "<b><font color='" .. cColor .. "'>[" .. drop.Rarity .. "] " .. drop.Name .. "</font></b><font color='#888888'>" .. pctStr .. "</font>"
				...
				for _, st in ipairs(statsOrder) do
					local sBox = Instance.new("Frame", statsArea); sBox.Size = UDim2.new(0, 50, 1, 0); sBox.BackgroundColor3 = Color3.fromRGB(15, 15, 20); Instance.new("UICorner", sBox).CornerRadius = UDim.new(0, 4); Instance.new("UIStroke", sBox).Color = Color3.fromRGB(40, 40, 50)
					local sName = Instance.new("TextLabel", sBox); sName.Size = UDim2.new(1, 0, 0.5, 0); sName.BackgroundTransparency = 1; sName.Font = Enum.Font.GothamMedium; sName.TextColor3 = Color3.fromRGB(150, 150, 150); sName.TextSize = 9; sName.Text = st.Name
					local sVal = Instance.new("TextLabel", sBox); sVal.Size = UDim2.new(1, 0, 0.5, 0); sVal.Position = UDim2.new(0, 0, 0.5, 0); sVal.BackgroundTransparency = 1; sVal.Font = Enum.Font.GothamBlack; sVal.TextColor3 = GetRankColor(st.Val); sVal.TextSize = 14; sVal.Text = st.Val
				end
			end
		else
			local sortedClans = {}
			...
			for _, drop in ipairs(sortedClans) do
				...
				local card = Instance.new("Frame", ListContainer); card.Size = UDim2.new(1, -10, 0, 70); card.BackgroundColor3 = Color3.fromRGB(22, 22, 28); Instance.new("UICorner", card).CornerRadius = UDim.new(0, 6)
				local stroke = Instance.new("UIStroke", card); stroke.Color = rarityRGB; stroke.Thickness = 1; stroke.Transparency = 0.55; stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
				local accentBar = Instance.new("Frame", card); accentBar.Size = UDim2.new(1, 0, 0, 3); accentBar.BackgroundColor3 = rarityRGB; accentBar.BorderSizePixel = 0
				local bgGlow = Instance.new("Frame", card); bgGlow.Size = UDim2.new(1, 0, 0.5, 0); bgGlow.Position = UDim2.new(0, 0, 0.5, 0); bgGlow.BackgroundColor3 = rarityRGB; bgGlow.BackgroundTransparency = 0.92; bgGlow.BorderSizePixel = 0; bgGlow.ZIndex = 1

				local titleLbl = Instance.new("TextLabel", card); titleLbl.Size = UDim2.new(1, -20, 0, 20); titleLbl.Position = UDim2.new(0, 10, 0, 10); titleLbl.BackgroundTransparency = 1; titleLbl.Font = Enum.Font.GothamBold; titleLbl.TextColor3 = Color3.fromRGB(230, 230, 230); titleLbl.TextSize = 15; titleLbl.TextXAlignment = Enum.TextXAlignment.Left
				...
				local descLbl = Instance.new("TextLabel", card); descLbl.Size = UDim2.new(1, -20, 0, 30); descLbl.Position = UDim2.new(0, 10, 0, 35); descLbl.BackgroundTransparency = 1; descLbl.Font = Enum.Font.GothamMedium; descLbl.TextColor3 = Color3.fromRGB(150, 255, 150); descLbl.TextSize = 12; descLbl.TextXAlignment = Enum.TextXAlignment.Left; descLbl.RichText = true; descLbl.Text = buffText
			end
		end
		...
		local storageBtns = {}
		for i = 1, 6 do
			local sBtn = Instance.new("TextButton", StorageArea); sBtn.BackgroundColor3 = Color3.fromRGB(22, 22, 28); sBtn.Font = Enum.Font.GothamBold; sBtn.TextColor3 = Color3.fromRGB(200, 200, 200); sBtn.TextScaled = true; Instance.new("UITextSizeConstraint", sBtn).MaxTextSize = 12; sBtn.TextWrapped = true; Instance.new("UICorner", sBtn).CornerRadius = UDim.new(0, 6)
		...
		local PityLbl = Instance.new("TextLabel", BottomArea); PityLbl.Size = UDim2.new(1, 0, 0, 20); PityLbl.Position = UDim2.new(0, 0, 0, 100); PityLbl.BackgroundTransparency = 1; PityLbl.Font = Enum.Font.GothamBold; PityLbl.TextColor3 = Color3.fromRGB(200, 150, 255); PityLbl.TextSize = 15; PityLbl.Text = "PITY: 0 / 100"
		...
		local RollBtn = Instance.new("TextButton", RollActions); RollBtn.Size = UDim2.new(0.3, 0, 1, 0); RollBtn.BackgroundColor3 = Color3.fromRGB(40, 80, 40); RollBtn.Font = Enum.Font.GothamBold; RollBtn.TextColor3 = Color3.fromRGB(255, 255, 255); RollBtn.TextScaled = true; Instance.new("UITextSizeConstraint", RollBtn).MaxTextSize = 12
		...
		local PremiumRollBtn = Instance.new("TextButton", RollActions); PremiumRollBtn.Size = UDim2.new(0.3, 0, 1, 0); PremiumRollBtn.BackgroundColor3 = Color3.fromRGB(150, 120, 40); PremiumRollBtn.Font = Enum.Font.GothamBold; PremiumRollBtn.TextColor3 = Color3.fromRGB(255, 255, 255); PremiumRollBtn.TextScaled = true; Instance.new("UITextSizeConstraint", PremiumRollBtn).MaxTextSize = 12; Instance.new("UICorner", PremiumRollBtn).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", PremiumRollBtn).Color = Color3.fromRGB(200, 150, 50)
		...
		local AutoRollBtn = Instance.new("TextButton", RollActions); AutoRollBtn.Size = UDim2.new(0.3, 0, 1, 0); AutoRollBtn.BackgroundColor3 = Color3.fromRGB(120, 60, 120); AutoRollBtn.Font = Enum.Font.GothamBold; AutoRollBtn.TextColor3 = Color3.fromRGB(255, 255, 255); AutoRollBtn.TextScaled = true; AutoRollBtn.Text = "ROLL TILL LEGENDARY+"; Instance.new("UITextSizeConstraint", AutoRollBtn).MaxTextSize = 12; Instance.new("UICorner", AutoRollBtn).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", AutoRollBtn).Color = Color3.fromRGB(150, 80, 150)
