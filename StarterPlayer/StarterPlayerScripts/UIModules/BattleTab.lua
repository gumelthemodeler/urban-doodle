...
function BattleTab.Init(parentFrame)
	...
	local CampImage = Instance.new("ImageLabel", CampContainer); CampImage.Size = UDim2.new(1, 0, 0.65, 0); CampImage.BackgroundColor3 = Color3.fromRGB(15, 15, 18); CampImage.Image = "rbxassetid://80153476985849"; CampImage.ScaleType = Enum.ScaleType.Crop; Instance.new("UICorner", CampImage).CornerRadius = UDim.new(0, 12)
	...
	SelectionCarousel = Instance.new("ScrollingFrame", SelectionContainer); SelectionCarousel.Size = UDim2.new(1, 0, 1, -60); SelectionCarousel.Position = UDim2.new(0, 0, 0, 60); SelectionCarousel.BackgroundTransparency = 1; SelectionCarousel.ScrollingDirection = Enum.ScrollingDirection.Y; SelectionCarousel.ScrollBarThickness = 6
	local sGridLayout = Instance.new("UIGridLayout", SelectionCarousel); sGridLayout.CellSize = UDim2.new(0, 280, 0, 420); sGridLayout.CellPadding = UDim2.new(0, 20, 0, 20); sGridLayout.SortOrder = Enum.SortOrder.LayoutOrder; sGridLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	local sPad = Instance.new("UIPadding", SelectionCarousel); sPad.PaddingTop = UDim.new(0, 10); sPad.PaddingBottom = UDim.new(0, 30)
	sGridLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() SelectionCarousel.CanvasSize = UDim2.new(0, 0, 0, sGridLayout.AbsoluteContentSize.Y + 40) end)
	...
	local function OpenSelectionMenu(titleText, dataTable, onPlayFunc)
		...
		for _, key in ipairs(sortedKeys) do
			...
			local card = Instance.new("TextButton", SelectionCarousel); card.LayoutOrder = layoutOrder; card.Text = ""; card.AutoButtonColor = false; card.BackgroundColor3 = Color3.fromRGB(15, 15, 18); card.ClipsDescendants = true; Instance.new("UICorner", card).CornerRadius = UDim.new(0, 12); local stroke = Instance.new("UIStroke", card); stroke.Color = Color3.fromRGB(60, 60, 70); stroke.Thickness = 2
			...
			local nLbl = Instance.new("TextLabel", card); nLbl.Size = UDim2.new(1, -20, 0, 25); nLbl.Position = UDim2.new(0, 10, 1, -125); nLbl.BackgroundTransparency = 1; nLbl.Font = Enum.Font.GothamBlack; nLbl.TextColor3 = Color3.fromRGB(255, 255, 255); nLbl.TextSize = 22; nLbl.TextScaled = true; nLbl.TextXAlignment = Enum.TextXAlignment.Left; nLbl.Text = data.Name or key
			local rLbl = Instance.new("TextLabel", card); rLbl.Size = UDim2.new(1, -20, 0, 20); rLbl.Position = UDim2.new(0, 10, 1, -100); rLbl.BackgroundTransparency = 1; rLbl.Font = Enum.Font.GothamBold; rLbl.TextColor3 = Color3.fromRGB(200, 200, 200); rLbl.TextSize = 14; rLbl.TextXAlignment = Enum.TextXAlignment.Left; rLbl.Text = "Req: Pres. " .. (data.Req or 0)
			local dLbl = Instance.new("TextLabel", card); dLbl.Size = UDim2.new(1, -20, 0, 35); dLbl.Position = UDim2.new(0, 10, 1, -80); dLbl.BackgroundTransparency = 1; dLbl.Font = Enum.Font.GothamMedium; dLbl.TextColor3 = Color3.fromRGB(180, 180, 180); dLbl.TextSize = 12; dLbl.TextXAlignment = Enum.TextXAlignment.Left; dLbl.TextYAlignment = Enum.TextYAlignment.Top; dLbl.TextWrapped = true; local descText = data.Desc or ("HP: " .. (data.Health or "Unknown")); if data.Drops then descText = descText .. " | Rewards: +" .. (data.Drops.XP or 0) .. " XP, +" .. (data.Drops.Dews or 0) .. " Dews" end; dLbl.Text = descText

			local pBtn = Instance.new("TextButton", card); pBtn.Size = UDim2.new(0.9, 0, 0, 35); pBtn.Position = UDim2.new(0.5, 0, 1, -5); pBtn.AnchorPoint = Vector2.new(0.5, 1); pBtn.Font = Enum.Font.GothamBlack; pBtn.TextSize = 16; pBtn.TextColor3 = Color3.fromRGB(255, 255, 255); pBtn.Text = "DEPLOY"; ApplyButtonGradient(pBtn, Color3.fromRGB(200, 60, 60), Color3.fromRGB(100, 30, 30), Color3.fromRGB(80, 20, 20)); pBtn.ZIndex = 5

			card.MouseEnter:Connect(function() TweenService:Create(uiScale, TweenInfo.new(0.15), {Scale = 1.03}):Play(); TweenService:Create(stroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(255, 100, 100)}):Play() end)
			card.MouseLeave:Connect(function() TweenService:Create(uiScale, TweenInfo.new(0.15), {Scale = 1.0}):Play(); TweenService:Create(stroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(60, 60, 70)}):Play() end)
	...
	ModesCarousel = Instance.new("ScrollingFrame", ModesContainer); ModesCarousel.Size = UDim2.new(1, 0, 1, 0); ModesCarousel.Position = UDim2.new(0, 0, 0, 0); ModesCarousel.BackgroundTransparency = 1; ModesCarousel.ScrollingDirection = Enum.ScrollingDirection.Y; ModesCarousel.ScrollBarThickness = 6
	local carouselLayout = Instance.new("UIGridLayout", ModesCarousel); carouselLayout.CellSize = UDim2.new(0, 280, 0, 420); carouselLayout.CellPadding = UDim2.new(0, 20, 0, 20); carouselLayout.SortOrder = Enum.SortOrder.LayoutOrder; carouselLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	local cPad = Instance.new("UIPadding", ModesCarousel); cPad.PaddingTop = UDim.new(0, 10)
	cPad.PaddingBottom = UDim.new(0, 80) -- [[ FIX: Massive padding to ensure the floating party button doesn't cover anything ]]

	local function CreateModeCard(name, desc, imgId, layoutOrder, onClickFunc)
		local card = Instance.new("TextButton", ModesCarousel); card.LayoutOrder = layoutOrder; card.Text = ""; card.AutoButtonColor = false; card.BackgroundColor3 = Color3.fromRGB(15, 15, 18); card.ClipsDescendants = true; Instance.new("UICorner", card).CornerRadius = UDim.new(0, 12); local stroke = Instance.new("UIStroke", card); stroke.Color = Color3.fromRGB(60, 60, 70); stroke.Thickness = 2
		...
		local title = Instance.new("TextLabel", card); title.Size = UDim2.new(1, -20, 0, 30); title.Position = UDim2.new(0, 10, 1, -90); title.BackgroundTransparency = 1; title.Font = Enum.Font.GothamBlack; title.TextSize = 22; title.TextColor3 = Color3.fromRGB(255, 255, 255); title.TextScaled = true; title.TextXAlignment = Enum.TextXAlignment.Left; title.Text = name
		local descLbl = Instance.new("TextLabel", card); descLbl.Size = UDim2.new(1, -20, 0, 40); descLbl.Position = UDim2.new(0, 10, 1, -55); descLbl.BackgroundTransparency = 1; descLbl.Font = Enum.Font.GothamBold; descLbl.TextSize = 13; descLbl.TextColor3 = Color3.fromRGB(180, 180, 180); descLbl.TextXAlignment = Enum.TextXAlignment.Left; descLbl.TextYAlignment = Enum.TextYAlignment.Top; descLbl.TextWrapped = true; descLbl.Text = desc

		card.MouseEnter:Connect(function() TweenService:Create(uiScale, TweenInfo.new(0.15), {Scale = 1.03}):Play(); TweenService:Create(stroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(255, 215, 100)}):Play() end)
		card.MouseLeave:Connect(function() TweenService:Create(uiScale, TweenInfo.new(0.15), {Scale = 1.0}):Play(); TweenService:Create(stroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(60, 60, 70)}):Play() end)
	...
	carouselLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() ModesCarousel.CanvasSize = UDim2.new(0, 0, 0, carouselLayout.AbsoluteContentSize.Y + 120) end)

	-- [[ FIX: Party Button is now a discreet floating button in the bottom right ]]
	PartyToggleBtn = Instance.new("TextButton", ModesContainer); PartyToggleBtn.Size = UDim2.new(0, 160, 0, 40); PartyToggleBtn.Position = UDim2.new(1, -20, 1, -20); PartyToggleBtn.AnchorPoint = Vector2.new(1, 1); PartyToggleBtn.Font = Enum.Font.GothamBlack; PartyToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255); PartyToggleBtn.TextSize = 14; PartyToggleBtn.Text = "PARTY"
