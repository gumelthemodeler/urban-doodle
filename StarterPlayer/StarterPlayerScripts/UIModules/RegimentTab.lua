...
function RegimentTab.Init(parentFrame, tooltipMgr)
	...
	Title.Size = UDim2.new(1, 0, 0, 40); Title.BackgroundTransparency = 1; Title.Font = Enum.Font.GothamBlack; Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.TextSize = 32; Title.Text = "REGIMENT WARS"
	ApplyGradient(Title, Color3.fromRGB(230, 230, 255), Color3.fromRGB(160, 180, 255))

	TimerLabel = Instance.new("TextLabel", MainFrame)
	TimerLabel.Size = UDim2.new(1, 0, 0, 20); TimerLabel.Position = UDim2.new(0, 0, 0, 45); TimerLabel.BackgroundTransparency = 1; TimerLabel.Font = Enum.Font.GothamBold; TimerLabel.TextColor3 = Color3.fromRGB(255, 150, 150); TimerLabel.TextSize = 16; TimerLabel.Text = "CYCLE ENDS IN: Awaiting Intel..."

	-- [[ FIX: Centered Tabs ]]
	local TopNav = Instance.new("Frame", MainFrame)
	TopNav.Size = UDim2.new(1, 0, 0, 45); TopNav.Position = UDim2.new(0, 0, 0, 75); TopNav.BackgroundTransparency = 1
	...
	ContentArea.Size = UDim2.new(1, 0, 1, -135); ContentArea.Position = UDim2.new(0, 0, 0, 135); ContentArea.BackgroundTransparency = 1

	local function CreateSubNavBtn(name, text)
		local btn = Instance.new("TextButton", TopNav)
		btn.Size = UDim2.new(0, 150, 0, 32); btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.fromRGB(180, 180, 180); btn.TextSize = 13; btn.Text = text
	...
	wmLayout.Padding = UDim.new(0, 0) -- Handled by explicit sizes

	local MapContainer = Instance.new("Frame", SubTabs["WarMap"])
	MapContainer.Size = UDim2.new(0.65, 0, 1, 0)
	MapContainer.BackgroundTransparency = 1
	MapContainer.LayoutOrder = 1

	-- [[ FIX: Removed Constraints & Used Crop to flawlessly fill the space ]]
	local MapFrame = Instance.new("ImageLabel", MapContainer)
	MapFrame.Size = UDim2.new(1, -15, 1, 0); MapFrame.Position = UDim2.new(0, 0, 0, 0)
	...
	PlayerRegLogo.Size = UDim2.new(0, 120, 0, 120)
	PlayerRegLogo.Position = UDim2.new(0, 15, 1, -15)
	...
	local MapTitle = Instance.new("TextLabel", MapFrame); MapTitle.Size = UDim2.new(1, 0, 0, 30); MapTitle.Position = UDim2.new(0, 0, 0, 15); MapTitle.BackgroundTransparency = 1; MapTitle.Font = Enum.Font.GothamBlack; MapTitle.TextColor3 = Color3.fromRGB(200, 220, 255); MapTitle.TextSize = 18; MapTitle.Text = "TACTICAL OVERVIEW"; MapTitle.ZIndex = 5

	local WallData = {
		{ Name = "Wall Maria", Scale = 0.85 },
		{ Name = "Wall Rose", Scale = 0.55 },
		{ Name = "Wall Sina", Scale = 0.30 }
	}
	...
	for _, w in ipairs(WallData) do
		...
		local str = Instance.new("UIStroke", wFrame); str.Color = Color3.fromRGB(100, 140, 220); str.Thickness = 3; str.Transparency = 0.6

		local wTxt = Instance.new("TextLabel", wFrame)
		wTxt.Size = UDim2.new(1, 0, 0, 20); wTxt.Position = UDim2.new(0.5, 0, 0, -15); wTxt.AnchorPoint = Vector2.new(0.5, 0.5); wTxt.BackgroundTransparency = 1; wTxt.Font = Enum.Font.GothamBlack; wTxt.Text = string.upper(w.Name); wTxt.TextColor3 = Color3.fromRGB(120, 160, 255); wTxt.TextTransparency = 0.4; wTxt.TextSize = 12; wTxt.ZIndex = 3
	end

	local DetailPanel = Instance.new("Frame", SubTabs["WarMap"])
	DetailPanel.Size = UDim2.new(0.35, 0, 1, 0); DetailPanel.BackgroundColor3 = Color3.fromRGB(16, 16, 20)
	...
	local dNameLbl = Instance.new("TextLabel", DetailPanel); dNameLbl.Size = UDim2.new(1, -30, 0, 35); dNameLbl.Position = UDim2.new(0, 15, 0, 15); dNameLbl.BackgroundTransparency = 1; dNameLbl.Font = Enum.Font.GothamBlack; dNameLbl.TextColor3 = Color3.fromRGB(255, 215, 100); dNameLbl.TextSize = 24; dNameLbl.TextXAlignment = Enum.TextXAlignment.Left; dNameLbl.Text = "AWAITING INTEL..."

	-- [[ FIX: Enabled RichText so the HTML color tags render properly ]]
	local dBuffLbl = Instance.new("TextLabel", DetailPanel); dBuffLbl.Size = UDim2.new(1, -30, 0, 40); dBuffLbl.Position = UDim2.new(0, 15, 0, 50); dBuffLbl.BackgroundTransparency = 1; dBuffLbl.Font = Enum.Font.GothamBold; dBuffLbl.TextColor3 = Color3.fromRGB(150, 150, 160); dBuffLbl.TextSize = 14; dBuffLbl.TextXAlignment = Enum.TextXAlignment.Left; dBuffLbl.TextYAlignment = Enum.TextYAlignment.Top; dBuffLbl.TextWrapped = true; dBuffLbl.RichText = true; dBuffLbl.Text = "Select a district on the map to review tactical data."

	local BarsFrame = Instance.new("Frame", DetailPanel); BarsFrame.Size = UDim2.new(1, -30, 0, 140); BarsFrame.Position = UDim2.new(0, 15, 0.4, 0); BarsFrame.BackgroundTransparency = 1
	local bLayout = Instance.new("UIListLayout", BarsFrame); bLayout.Padding = UDim.new(0, 12)

	local DeployBtn = Instance.new("TextButton", DetailPanel)
	DeployBtn.Size = UDim2.new(1, -30, 0, 55); DeployBtn.Position = UDim2.new(0.5, 0, 1, -20); DeployBtn.AnchorPoint = Vector2.new(0.5, 1)
	DeployBtn.Font = Enum.Font.GothamBlack; DeployBtn.TextColor3 = Color3.fromRGB(255, 255, 255); DeployBtn.TextSize = 16; DeployBtn.Text = "DEPLOY FORCES"
	...
	for i, reg in ipairs({"Garrison", "Military Police", "Scout Regiment"}) do
		local barBg = Instance.new("Frame", BarsFrame); barBg.Size = UDim2.new(1, 0, 0, 30); barBg.BackgroundColor3 = Color3.fromRGB(10, 10, 12); Instance.new("UICorner", barBg).CornerRadius = UDim.new(0, 6)
		Instance.new("UIStroke", barBg).Color = Color3.fromRGB(40, 40, 50); barBg.UIStroke.Thickness = 1

		local fill = Instance.new("Frame", barBg); fill.Size = UDim2.new(0, 0, 1, 0); fill.BackgroundColor3 = Color3.fromRGB(255, 255, 255); Instance.new("UICorner", fill).CornerRadius = UDim.new(0, 6)
		...
		local txt = Instance.new("TextLabel", barBg); txt.Size = UDim2.new(1, -15, 1, 0); txt.Position = UDim2.new(0, 10, 0, 0); txt.BackgroundTransparency = 1; txt.Font = Enum.Font.GothamBlack; txt.TextColor3 = Color3.fromRGB(255, 255, 255); txt.TextSize = 13; txt.TextXAlignment = Enum.TextXAlignment.Left; txt.Text = string.upper(reg) .. " - 0 VP"; txt.ZIndex = 2
	...
	for dName, dInfo in pairs(RegimentData.Districts) do
		...
		lblBg.Size = isCentered and UDim2.new(0, 130, 0, 22) or UDim2.new(0, 120, 0, 22)
		lblBg.Position = dInfo.LabelPos or UDim2.new(0.5, 0, 1, 8)
		lblBg.AnchorPoint = dInfo.LabelAnchor or Vector2.new(0.5, 0)
		lblBg.BackgroundColor3 = Color3.fromRGB(10, 10, 14); lblBg.BackgroundTransparency = 0.2; lblBg.ZIndex = 10
		Instance.new("UICorner", lblBg).CornerRadius = UDim.new(0, 4)

		local lblAccent = Instance.new("Frame", lblBg)
		lblAccent.Size = UDim2.new(0, 3, 1, 0); lblAccent.Position = UDim2.new(0, 0, 0, 0); lblAccent.BackgroundColor3 = Color3.fromRGB(150, 150, 160); lblAccent.BorderSizePixel = 0; lblAccent.ZIndex = 11
		Instance.new("UICorner", lblAccent).CornerRadius = UDim.new(0, 4)

		if not isCentered then
			local pad = Instance.new("UIPadding", lblBg); pad.PaddingLeft = UDim.new(0, 10); pad.PaddingRight = UDim.new(0, 6)
		end

		local lbl = Instance.new("TextLabel", lblBg)
		lbl.Size = UDim2.new(1, 0, 1, 0); lbl.BackgroundTransparency = 1; lbl.Font = Enum.Font.GothamBold; lbl.TextColor3 = Color3.fromRGB(200, 200, 210); lbl.TextSize = 12; lbl.Text = string.upper(dName); lbl.ZIndex = 11; lbl.TextXAlignment = dInfo.TextAlign or Enum.TextXAlignment.Center
		...
		nodeBtn.MouseButton1Click:Connect(function()
			...
			dNameLbl.Text = string.upper(dName); dBuffLbl.Text = "<font color='#55FF55'>Strategic Value:</font>\n" .. dInfo.Buff
			...
			for n, data in pairs(dMapNodes) do
				...
				if n == dName then
					...
					data.Tween = TweenService:Create(data.SelRing, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, -1), {Size = UDim2.new(1, 24, 1, 24), UIStroke = {Transparency = 1}})
					data.Tween:Play()
	...
	SubTabs["Factions"].Size = UDim2.new(1, 0, 1, 0); SubTabs["Factions"].BackgroundTransparency = 1; SubTabs["Factions"].Visible = false; SubTabs["Factions"].ScrollBarThickness = 6
