...
function RaidTab.Init(parentFrame, tooltipMgr)
	...
	-- [[ FIX: Constrained exact size for PC PvP-style look ]]
	ArenaFrame = Instance.new("Frame", parentFrame.Parent)
	ArenaFrame.Name = "RaidArenaFrame"
	ArenaFrame.Size = UDim2.new(0, 750, 0, 520) 
	ArenaFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
	ArenaFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	ArenaFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
	ArenaFrame.Visible = false
	ArenaFrame.ZIndex = 200
	...
	-- [[ DYNAMIC PARTY AREA ]]
	CombatantsFrame = Instance.new("Frame", ArenaFrame); CombatantsFrame.Size = UDim2.new(0.96, 0, 0, 85); CombatantsFrame.BackgroundTransparency = 1; CombatantsFrame.LayoutOrder = 2
	...
	eAvatarBox = Instance.new("Frame", BossFrame)
	eAvatarBox.Size = UDim2.new(0, 80, 0, 80) -- [[ FIX: Increased icon size for PC ]]
	eAvatarBox.Position = UDim2.new(1, 0, 0.5, 0)
	eAvatarBox.AnchorPoint = Vector2.new(1, 0.5)
	eAvatarBox.BackgroundColor3 = Color3.fromRGB(10, 10, 15)
	...
	eStatsArea = Instance.new("Frame", BossFrame); eStatsArea.Size = UDim2.new(1, -90, 0, 70); eStatsArea.Position = UDim2.new(0, 0, 0.5, 0); eStatsArea.AnchorPoint = Vector2.new(0, 0.5); eStatsArea.BackgroundTransparency = 1
	...
	BossNameText = Instance.new("TextLabel", eStatsArea); BossNameText.Size = UDim2.new(1, 0, 0, 12); BossNameText.BackgroundTransparency = 1; BossNameText.Font = Enum.Font.GothamBlack; BossNameText.TextColor3 = Color3.fromRGB(255, 120, 120); BossNameText.TextSize = 14; BossNameText.TextScaled = true; BossNameText.TextXAlignment = Enum.TextXAlignment.Right

	local eHpCont
	BossHPBar, BossHPText, eHpCont = CreateBar(eStatsArea, Color3.fromRGB(220, 60, 60), Color3.fromRGB(140, 30, 30), UDim2.new(1, 0, 0, 14), "HP: 100", true)
	...
	local FeedBox = Instance.new("Frame", ArenaFrame); FeedBox.Size = UDim2.new(0.96, 0, 0, 70); FeedBox.BackgroundColor3 = Color3.fromRGB(22, 22, 26); FeedBox.ClipsDescendants = true; FeedBox.LayoutOrder = 3
	...
	LogText = Instance.new("TextLabel", FeedBox); LogText.Size = UDim2.new(1, -20, 1, -10); LogText.Position = UDim2.new(0, 10, 0, 5); LogText.BackgroundTransparency = 1; LogText.Font = Enum.Font.GothamMedium; LogText.TextColor3 = Color3.fromRGB(230, 230, 230); LogText.TextSize = 14; LogText.TextXAlignment = Enum.TextXAlignment.Left; LogText.TextYAlignment = Enum.TextYAlignment.Top; LogText.TextWrapped = true; LogText.RichText = true; LogText.Text = ""

	BottomArea = Instance.new("Frame", ArenaFrame)
	BottomArea.Size = UDim2.new(0.96, 0, 0, 180) -- [[ FIX: Fixed height for PC Bottom area ]]
	...
	-- [[ FIX: Fixed PC Grid Box Size ]]
	gridLayout.CellSize = UDim2.new(0, 170, 0, 45)
	...
	local function BuildParty(partyData)
		...
		CombatantsFrame.Size = UDim2.new(0.96, 0, 0, cHeight)
		BottomArea.Size = UDim2.new(0.96, 0, 0, 180) 
		...
		for _, mem in ipairs(pData) do
			...
			local nameLbl = Instance.new("TextLabel", pStatsArea); nameLbl.Size = UDim2.new(1, 0, 0, 14); nameLbl.BackgroundTransparency = 1; nameLbl.Font = Enum.Font.GothamBlack; nameLbl.TextColor3 = (mem.UserId == player.UserId) and Color3.fromRGB(150, 200, 255) or Color3.fromRGB(200, 220, 255); nameLbl.TextSize = 12; nameLbl.TextXAlignment = Enum.TextXAlignment.Left; nameLbl.TextScaled = true; nameLbl.Text = string.upper(mem.Name)

			local hpBar, hpTxt = CreateBar(pStatsArea, Color3.fromRGB(220, 60, 60), Color3.fromRGB(140, 30, 30), UDim2.new(1, 0, 0, 12), "HP: " .. math.floor(mem.HP) .. " / " .. math.floor(mem.MaxHP), false)
			local gasBar, gasTxt = CreateBar(pStatsArea, Color3.fromRGB(150, 220, 255), Color3.fromRGB(60, 140, 200), UDim2.new(1, 0, 0, 10), "GAS: " .. math.floor(mem.Gas) .. " / " .. math.floor(mem.MaxGas), false)
